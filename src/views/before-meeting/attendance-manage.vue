        ...
        <el-main class="el-flex is-column">
            <div class="el-bg--white p-20px tools-wrapper">
                <div class="el-flex is-center-between mb-30px">
                    <div>
                        <el-button type="primary">一键全签</el-button>
            ...
            <div class="el-bg--white p-20px mt-20px list-wrapper">
                <b-grid ref="refGrid" v-bind="gridProps">
                    ...
                    <template #actions="{ row }">
                        <el-button type="primary" size="small" @click="onEdit(row)">修改状态</el-button>
                    </template>
                </b-grid>
            </div>
        </el-main>
...
import { dropDownAttendanceStatus, getAttendPage } from '@/api/before-meeting/attendance-manage.ts'
...
function init() {
    ...
    dropDownAttendanceStatus().then((res) => {
        dropAttendanceStatus.value = dropDownSetValueNumner(res.data, true, true)
        formModel.value.status = dropAttendanceStatus.value.length > 0 ? dropAttendanceStatus.value[0].value : null
    })
}
init()
</script>
