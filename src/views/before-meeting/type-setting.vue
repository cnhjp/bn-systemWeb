<template>
    <el-container class="wh-full">
        <el-header>
            <page-header title="类型配置" is-show-btn></page-header>
        </el-header>
...
import { getTypeSettingList, deleteTypeSetting } from '~/src/api/before-meeting/info'
import dialogTypeSetting from './components/dialog-type-setting.vue'
...
const gridProps = reactive({
    ...
    columns: [
        ...
        { title: '排序', field: 'sortIndex', minWidth: 120, align: 'center' },
...
function onDelete(row: any) {
    ElMessageBox.confirm('确定删除该类型吗？').then(() => {
        deleteTypeSetting(row.conventionTypeId).then(() => {
